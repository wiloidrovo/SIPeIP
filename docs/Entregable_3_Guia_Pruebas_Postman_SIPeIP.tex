...
\atencion{No valide todavía la alineación con otro perfil. Debe permanecer en BORRADOR para que el Supervisor la revise dentro del expediente del plan en la Prueba 8.}

\section{Prueba 7: preparar Plan, Meta e Indicador}
...
\esperado{Status \textbf{201}. Copie el ID del indicador. Su \path{valor_actual} debe coincidir con la línea base 0.00, \path{sentido} debe ser ASCENDENTE, \path{ponderacion} debe ser 100.00 y \path{validado} debe ser falso.}
...
\section{Prueba 8: revisar el contenido, validar y aprobar el mismo plan}

Cambie al usuario \path{supervisor}. Use el \path{<ID_PLAN>}, \path{<ID_ALINEACION>} e \path{<ID_INDICADOR>} creados en las pruebas anteriores.

\subsection{REV-01 --- Abrir el expediente antes de decidir}

Cree una solicitud \textbf{GET}, sin Body ni headers agregados manualmente:

\begin{lstlisting}
http://localhost:8000/api/planes/<ID_PLAN>/expediente/
\end{lstlisting}

\esperado{Status \textbf{200}. Antes de tomar una decisión, compruebe en el JSON: la descripción ``Plan creado para comprobar el flujo completo'', el objetivo OEI-GUIA, la meta, el indicador, el objetivo PND y el ODS 16. En \path{validacion} debe aparecer que todavía faltan las validaciones de la alineación y del indicador.}

\subsection{REV-02 --- Tomar la revisión}

\begin{enumerate}
  \item Cree una solicitud \textbf{POST} a \path{http://localhost:8000/api/planes/<ID_PLAN>/revisar/}.
  \item En \textbf{Headers} agregue \path{Content-Type = application/json} y \path{X-CSRFToken = token actual}.
  \item En \textbf{Body $>$ raw $>$ JSON} escriba:
\end{enumerate}
...
\begin{lstlisting}
{
  "observacion": "Se revisaron la descripcion, la meta, el indicador y la alineacion."
}
\end{lstlisting}

\esperado{Status \textbf{200}, estado EN\_REVISION\_INICIADA y \path{revisor_detalle.username: "supervisor"}.}

\subsection{REV-03 --- Validar la alineación del expediente}

\begin{enumerate}
  \item Cree una solicitud \textbf{POST} a \path{http://localhost:8000/api/alineaciones/<ID_ALINEACION>/validar/}.
  \item Agregue \path{Content-Type = application/json} y \path{X-CSRFToken = token actual}.
  \item En \textbf{Body $>$ raw $>$ JSON} escriba \texttt{\{\}}.
  \item Pulse \textbf{Send}.
\end{enumerate}

\esperado{Status \textbf{200}, estado VALIDADA y \path{usuario_validador_detalle.username: "supervisor"}. Esta relación hace que el ODS y el PND aparezcan en las metas e indicadores que contribuyen al objetivo estratégico.}

\subsection{REV-04 --- Validar la ficha del indicador}

\begin{enumerate}
  \item Cree una solicitud \textbf{POST} a \path{http://localhost:8000/api/indicadores/<ID_INDICADOR>/validar/}.
  \item Mantenga los dos headers y el Body \texttt{\{\}}.
  \item Pulse \textbf{Send}.
\end{enumerate}

\esperado{Status \textbf{200}, \path{validado: true}, \path{validado_por_detalle.username: "supervisor"} y una fecha de validación.}

\subsection{REV-05 --- Confirmar que el expediente ya puede aprobarse}

Cree una solicitud \textbf{GET}, sin Body:

\begin{lstlisting}
http://localhost:8000/api/planes/<ID_PLAN>/validacion/
\end{lstlisting}

\esperado{Status \textbf{200}, \path{listo_para_aprobacion: true} y una lista vacía en \path{bloqueos_aprobacion}. Si todavía existe un bloqueo, corríjalo antes de continuar; no fuerce el estado del plan con PATCH.}

\subsection{REV-06 --- Aprobar después de revisar el contenido}

Cree una solicitud \textbf{POST} a:

\begin{lstlisting}
http://localhost:8000/api/planes/<ID_PLAN>/aprobar/
\end{lstlisting}

Use los dos headers y este Body:

\begin{lstlisting}
{
  "observacion": "Expediente completo, alineacion e indicador validados."
}
\end{lstlisting}

\esperado{Status \textbf{200}, estado APROBADO, \path{aprobado_por_detalle.username: "supervisor"} y fecha de aprobación.}

\subsection{AVA-01 --- Comprobar que un avance negativo se rechaza}

Cambie nuevamente a \path{planificador}. El avance se prueba ahora, porque SIPeIP solo permite medir indicadores validados de planes aprobados.
...
\esperado{Status \textbf{400}. No se crea ningún avance ni cambia el valor actual.}

\subsection{AVA-02 --- Registrar una medición con evidencia}

Mantenga la misma URL y reemplace el Body:

\begin{lstlisting}
{
  "fecha_registro": "2026-07-15",
  "valor": "25.00",
  "observacion": "Primer avance del indicador aprobado.",
  "evidencia": "Acta interna de medicion GUIA-01"
}
\end{lstlisting}

\esperado{Status \textbf{201}. El registrador debe ser \path{planificador}, aunque ese campo no se envió. El valor actual pasa a 25.00 y la evidencia queda en el historial.}

\subsection{AVA-03 --- Consultar el seguimiento calculado}

Cree estas dos solicitudes \textbf{GET}, sin Body ni headers manuales:

\begin{lstlisting}
http://localhost:8000/api/indicadores/<ID_INDICADOR>/seguimiento/
http://localhost:8000/api/planes/<ID_PLAN>/seguimiento/
\end{lstlisting}

\esperado{Ambas responden \textbf{200}. El indicador muestra línea base 0, valor actual 25, meta 100, progreso 25 por ciento, próxima medición, estado de seguimiento, ODS 16 y la serie de avances. El plan consolida ese resultado dentro de su objetivo y meta; el valor ya no está suelto.}

Para comprobar la trazabilidad del flujo cree una solicitud \textbf{GET}:

\begin{lstlisting}
http://localhost:8000/api/planes/<ID_PLAN>/historial/
\end{lstlisting}

\esperado{Aparecen al menos ENVIAR\_REVISION, INICIAR\_REVISION y APROBAR, con sus usuarios, fechas, estados anterior/nuevo y observaciones.}
